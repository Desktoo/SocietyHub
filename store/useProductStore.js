...
export const useProductStore = create( persist((set, get) => ({
    ...
    setCategory: (category) => {
        const { products } = get()
        const filtered = category === "All Categories" ? products : products.filter(item => item.category?.toLowerCase() === category.toLowerCase())
    ...
    addToWishlist: (product) => {
        ...
        const isAlreadyPresent = wishlist.some(item => item.id === product.id)
