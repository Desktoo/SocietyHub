...
export const useSellStore = create((set, get) => ({
    ...
    formSubmit: async (formData) => {
        try {
            ...
            toast.error("Failed to Uplaod Product")
            return null
        }
    },
    ...
    deleteProduct: async (productId) => {
        try {
            ...
            const updatedList = get().productList.filter(product => product._id !== productId);
    ...
    updateProduct: async (productId, updatedData) => {
        try {
            ...
            const res = await axiosInstance.patch(`/products/${productId}`, updatedData, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
