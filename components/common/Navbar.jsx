...
const Navbar = () => {
    ...
    const SearchBar = ({ isMobile = false }) => (
        <div className={`${isMobile ? 'w-full mt-4 md:hidden' : 'w-md lg:w-xl hidden md:block'}`}>
    ...
    const AuthButtons = ({ isMobile = false }) => (
        isLoggedIn ? (
            <motion.div className='lg:flex-row flex-col flex gap-5 lg:items-center'>
                ...
                >
                    <Store className='w-5' />
                    <label className='lg:hidden font-semibold text-md'>Sell Products</label>
                </motion.div>
                </Link>
            ...
            <div className={`flex ${isMobile ? 'flex-col gap-2 mt-2' : 'gap-2 md:gap-4 flex items-center'}`}>
                <Link href='/login'>
                    <motion.button 
                        className={`${isMobile ? 'border border-neutral-400 rounded-lg py-2 px-4' : ''} font-semibold cursor-pointer text-neutral-600`}
                ...
                <Link href='/signup'>
                    <motion.div 
                        ...
                        transition={{ duration: 0.2 }}
                    >
                        <button className='text-white font-semibold cursor-pointer'>Signup</button>
                    </motion.div>
                </Link>
            </div>
        )
