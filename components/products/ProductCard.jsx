  ...
  return (
        ...
        >
            <div className='relative object-contain w-full h-56  flex '>
                <Image src={product.imageUrl && product.imageUrl.trim() ? product.imageUrl : imgNotFound} alt={product.productName} fill className='object-cover' />
            </div>
            <div className='p-5 bg-white w-full rounded-b-lg flex flex-col '>
                <div className='flex w-full justify-between'>
                    <label className="text-left font-bold text-lg md:text-xl text-gray-800">{product.productName}</label>
                    <motion.div 
                        className={`p-2 rounded-full cursor-pointer ${like ? 'bg-red-500' : 'bg-white border border-gray-500'}`}
                        onClick={handleWishlistClick}
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                    >
                        <Heart className={like ? 'text-white fill-white' : 'text-gray-500'} /> 
                    </motion.div>
                </div>
                ...
                <div className='flex gap-2'>
                    <motion.div 
                        ...
                        whileTap={{ scale: 0.95 }}
                    >
                        <button className='text-sm md:text-base  text-gray-700'>Add to cart</button>
                    </motion.div>
